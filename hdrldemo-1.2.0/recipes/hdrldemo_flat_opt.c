...
/*----------------------------------------------------------------------------*/
/**
 * @defgroup hdrldemo_flat_opt  master flat creation in the optical regime
 * @par Synopsis: TBD
...
#define RECIPE_NAME "hdrldemo_flat_opt"

static char hdrldemo_flat_opt_description[] =
"                                                                           \n"
"The recipe derives a master flatfield in the optical regime.               \n"
"                                                                           \n"
"Input files:                                                               \n"
"                                                                           \n"
...
"                                                                           \n"
"                                                                           \n"
"Please note that if needed, the input frames can be overscan corrected     \n"
"(see recipe parameter --oscan.) and bias corrected - given there is a      \n"
"master bias frame (MASTER_BIAS) in the SOF. Moreover, the errors of        \n"
"the input frames (FLAT) are estimated with a shot noise model by using     \n"
"the Gain (--gain) and Ron (--ron).                                         \n"
"                                                                           \n"
...
cpl_recipe_define(hdrldemo_flat_opt,
                    HDRLDEMO_BINARY_VERSION,
                    "HDRL Group",
                    PACKAGE_BUGREPORT,
                    "2013",
                    "HDRLDEMO - Master Flat creation in the optical regime",
                    hdrldemo_flat_opt_description);

/* Function needed by cpl_recipe_define to fill the input parameters */
static cpl_error_code hdrldemo_flat_opt_fill_parameterlist(
        ...
        const cpl_parameterlist *   parlist)
{
    ...
    if (flat_params == NULL) {
        if (os_params) hdrl_parameter_destroy(os_params) ;
    ...
    for (cpl_size i = 0; i < fsize; i++) {
        ...
        } else if (master_bias == NULL && !strcmp(cpl_frame_get_tag(cur_frame),
                        HDRLDEMO_MASTER_BIAS)) {
            cpl_msg_info(cpl_func,"Masterbias found in the SOF");
            master_bias = cpl_image_load(cpl_frame_get_filename(cur_frame),
                            CPL_TYPE_DOUBLE, 0, 0);
        } else if (master_bias_err == NULL && !strcmp(cpl_frame_get_tag(cur_frame),
                        HDRLDEMO_MASTER_BIAS_ERROR)) {
            cpl_msg_info(cpl_func,"Masterbias error found in the SOF");
    ...
    /* TODO leftover from masterbias - eliminate */
    ...
    /*Save the results*/
        ...
        hdrldemo_save_image(NULL, NULL, HDRLDEMO_MASTER_FLAT_CONTRIBUTION,
        	"hdrldemo_flat_opt", "hdrldemo_masterflat_contribution.fits",
